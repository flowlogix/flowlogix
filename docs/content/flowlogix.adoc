...
* Easier-to-use, Injected JPA Lazy Data Model for PrimeFaces DataTable
...
PrimeFaces provides a convenient https://www.javadoc.io/doc/org.primefaces/primefaces/latest/org/primefaces/model/JpaLazyDataModel.html[wrapper^] for the Lazy DataModel. However, FlowLogix `JPALazyDataModel` predates it and thus has a big "head start" in features, compactness and ease of use. Biggest advantage is ability to easily `@Inject` the model via CDI. The model also utilizes `DaoHelper` classes and methodology to make JPA lazy data model even easier, with a lot less code and better design. It also does not require inheritance and problems associated with it.
[source,xhtml]
.userviewer.xhtml
----
