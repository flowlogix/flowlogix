...
- `glassfish-pom`: A pre-configured environment for running integration tests with Payara Server.
- `server-common`: Common base POM for Payara and GlassFish Servers.
...
=== `payara-pom` and `glassfish-pom`

The `payara-pom` and `glassfish-pom` modules provides a pre-configured environment for running integration tests with Payara or GlassFish Server. It offers several profiles to support different testing scenarios:

- `server-local`: For running tests against a local Payara or GlassFish Server instance.
- `server-remote`: For running tests against a remote Payara or GlassFish Server instance.
- `server-docker`: For running tests in a Docker container.
...
You can also run the tests and generate the report locally. To configure Payara or GlassFish for JaCoCo, download and run the initialization script link:https://github.com/flowlogix/base-pom/blob/main/server-common/jacoco-init.sh[jacoco-init.sh]:

[source,shell]
.Run the JaCoCo initialization script for Payara or GlassFish
----
curl -sSL https://raw.githubusercontent.com/flowlogix/base-pom/main/server-common/jacoco-init.sh | bash -p
----
...
== `payara-pom` and `glassfish-pom` Variables

The `payara-pom` and `glassfish-pom` module define the following properties:

|===
| Variable | Description

| `server.start.disable`
...
| `glassfish.imageName`
| The name of the GlassFish image to use.

| `server.asadmin.executable.suffix`
| The suffix for the `asadmin` executable.

| `server.asadmin.executable`
