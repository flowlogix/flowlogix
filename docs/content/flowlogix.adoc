...
----
The `@NonDefault` can be removed to use it as a Default producer. If there is only one Persistence Unit, `unitName` can be also omitted.
[source,java]
...
* entityManager: Provide entity manager `Supplier`. Fefault is injected via CDI
* entityManagerQualifiers: Provides a list of qualifiers annotations to select the correct EntityManager for injection
